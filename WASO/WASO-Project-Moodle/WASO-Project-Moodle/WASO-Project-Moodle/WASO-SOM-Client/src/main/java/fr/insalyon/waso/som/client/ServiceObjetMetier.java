...
public class ServiceObjetMetier {
    ...
    public void rechercherClientParDenomination(String denomination,String ville)throws ServiceException {
        try {
            ...
            List<Object[]> listeClients = this.dBConnection.launchQuery("SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM CLIENT WHERE UPPER(Denomination) like '%' || UPPER(?) || '%' AND UPPER(Ville) like '%' || UPPER(?) || '%' ORDER BY ClientID", denomination, ville);
