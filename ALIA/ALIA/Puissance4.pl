...
%Recuperer l'element d'une liste a l'indice N1 et le resultat va dans X
%Recuperer les indices de l'element X et le resultat va dans N1
...
%Afficher les separations entre les lignes de la grille
...
%Si le jeu est termine, on affiche le gagnant
...
jouer(G,J,LCP) :-afficherSeparation, afficherPlateau(G),
    ...
    jouerAvecHeuristique(G,LCP,J,C), %Selectionner le coup a jouer grace a une heuristique
    write('Coup choisi : '),
    writeln(C),
...
jouerAvecHeuristique(G,LCP,J,C) :-  J==1, heuristique3(G,LCP,J,C), writeln(C). %Le joueur 1 joue avec l'heuristique attaque et defense orientee attaque
jouerAvecHeuristique(G,LCP,J,C) :- J==2, heuristique4(G,LCP,J,C). %2 joue avec l'heuristique attaque/defense

%modifier LCP apres avoir joue un coup
%trouver le coup possible suivant s'il existe
%incremente(nouveau coup determine à partir du coup joue, coup joue)
increment_CP([Z|Y],[X|Y]):- X>0, Z is X-1.
increment_CP([],[0|_]).

%supprimer le coup deja joue de la liste des coups possibles
supprimer_CP(C,[C|L],R):- supprimer_CP(C,L,R).
supprimer_CP(C,[X|L],[X|R]) :- supprimer_CP(C,L,R) ,X\==C.
supprimer_CP(_,[],[]).

%Ajouter le nouveau coup a la liste des coups possibles
...
%Remplacer l'element a l'indice I d'une liste L par X et mettre la nouvelle liste dans R
remplacer([_|L],0,X,[X|L]) :- !.
remplacer([H|L],I,X,[H|R]) :- I>0, I1 is I-1, remplacer(L,I1,X,R). 
...
jouerCoup(G,C,NouvG,J) :- recupIndLigne(C, IndLigne), recupIndColonne(C, IndColonne),
    nth0(IndLigne, G, L), %Recuperation de la ligne a jouer
...
%%Victoire a l'horizontale
%Regarder s'il y a une combinaison de 4 jetons J sur une meme ligne, le resultat est renvoye dans le dernier argument
...
%Recuperer chaque element à l'indice I de toutes les lignes L de la grille G pour en faire une colonne R
recupererCol([],_,[]).
recupererCol([L|G],I,[R1|R]):-trouverElement(I,R1,L),recupererCol(G,I,R).

%Recuperer une colonne apres l'autre et regarder si elle est gagnante, sinon continuer avec la colonne suivante
...
%%Victoire a la diagonale
%%Victoire en diagonale sens 1 : haut droit et bas gauche
%Parcourir la grille G et recuperer l'element a l'indice [I][K]. Si il est egal à J alors, on incremente notre compteur CMPT et on poursuit
%la recherche dans la même diagonale. Sinon, on met CMPT a 0 et on continue dans la même diagonale. Dans les deux cas, on incremente CMTI et CMTK
%qui correspondent au nombre de coups faits dans une meme diagonale.
...
%Lorsque on depasse les limites de la grille, on se replace au début de la diagonale suivante.
...
%Si CMTP = 4, alors la diagonale est victorieuse et on arrête.
%Si on a fini le parcours de toute la grille, on arrete et on retourne fail.
%Lorsque on depasse les limites de la grille, on se replace au debut de la diagonale suivante.
...
%%Grille pleine
%On regarde si la liste des coups possibles LCP est vide. Si c'est le cas la partie est terminee.
ligneVide([]).
grillePleine([]).
